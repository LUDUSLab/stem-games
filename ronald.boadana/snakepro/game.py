...
def game_loop():
    ...
    while game_on:
        game_clock.tick(game_difficult)
        ...
        if direction == UP:
            ...
            snake[0] = snake_head_pos = (snake[0][0], snake[0][1] - 32)
        if direction == DOWN:
            for i in range(len(snake) - 1, 0, -1):
                snake[i] = (snake[i - 1][0], snake[i - 1][1])
            snake[0] = snake_head_pos = (snake[0][0], snake[0][1] + 32)
        if direction == RIGHT:
            for i in range(len(snake) - 1, 0, -1):
                snake[i] = (snake[i - 1][0], snake[i - 1][1])
            snake[0] = snake_head_pos = (snake[0][0] + 32, snake[0][1])
        if direction == LEFT:
            for i in range(len(snake) - 1, 0, -1):
                snake[i] = (snake[i - 1][0], snake[i - 1][1])
            snake[0] = snake_head_pos = (snake[0][0] - 32, snake[0][1])
        ...
        # updating score
        score_text = score_font.render('SCORE:' + str(player_score), True, color_black, color_orange)
